...
class PassageRE(nn.Module):
    ...
    def __init__(self,
        ...
        self.l1loss = torch.nn.SmoothL1Loss(reduction='sum')
    ...
    def train_model(self, metric='auc'):
        ...
        for epoch in range(self.max_epoch):
            ...
            for iter, data in enumerate(t):
                ...
                rel_scores, mu, logvar, rep_rel, rep_cls, rep_recon = self.model(token, mask)
 
                label = torch.stack(label).to(self.device)
                loss1 = self.criterion(rel_scores, label)
                loss2 = -0.5 * torch.sum(logvar + 1 - mu.pow(2) - logvar.exp())
                loss3 = self.l1loss(rep_cls, rep_recon)
                loss = 0.9*loss1 + 0.1*loss2 + 0.001*loss3
                pred = (rel_scores >= 0.5) * torch.tensor([[1] * rel_scores.shape[1]] * rel_scores.shape[0]).to(
                    self.device)
    ...
    def eval_model(self, eval_loader):
        ...
        with torch.no_grad():
            ...
            for iter, data in enumerate(t):
                ...
                token, mask = data[2].squeeze(1), data[3].squeeze(1)
                logits, mu, logvar, rep_rel, rep_cls, rep_recon= self.model(token, mask, False)
